...
export const vacunacionRouter = new Hono<HonoEnv>()
...
vacunacionRouter.get("/pacientes/:pacienteId/vacunaciones", async (c) => {
...
vacunacionRouter.post("/pacientes/:pacienteId/vacunaciones", async (c) => {
  const cId = await getConsultorioId(c.get("tenantId"))
  if (!cId) return c.json({ error: "CONSULTORIO_NO_ENCONTRADO" }, 404)
...
vacunacionRouter.delete("/pacientes/:pacienteId/vacunaciones/:id", async (c) => {
  const cId = await getConsultorioId(c.get("tenantId"))
  if (!cId) return c.json({ error: "CONSULTORIO_NO_ENCONTRADO" }, 404)
  try {
    await makePacienteRepo().obtener(c.req.param("pacienteId"), cId)
