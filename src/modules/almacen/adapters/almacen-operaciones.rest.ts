...
almacenOperacionesRouter.get("/ingresos/:ingresoId", async (c) => {
  const tenantId = c.get("tenantId")
  try {
...
almacenOperacionesRouter.patch("/ingresos/:ingresoId", requireRol(["PROPIETARIO", "ADMIN"]), async (c) => {
  const tenantId = c.get("tenantId")
  const session = c.get("session")
  const body = await c.req.json()
...
almacenOperacionesRouter.post("/ingresos/:ingresoId/aprobar", requireRol(["PROPIETARIO", "ADMIN"]), async (c) => {
  const tenantId = c.get("tenantId")
  const session = c.get("session")
  const body = await c.req.json()
...
almacenOperacionesRouter.get("/salidas/:salidaId", async (c) => {
  const tenantId = c.get("tenantId")
  try {
...
almacenOperacionesRouter.patch("/salidas/:salidaId", requireRol(["PROPIETARIO", "ADMIN"]), async (c) => {
  const tenantId = c.get("tenantId")
  const session = c.get("session")
  const body = await c.req.json()
...
almacenOperacionesRouter.post("/salidas/:salidaId/aprobar", requireRol(["PROPIETARIO", "ADMIN"]), async (c) => {
  const tenantId = c.get("tenantId")
  const session = c.get("session")
  const body = await c.req.json()
  const parsed = AprobarDocumentoSchema.safeParse(body)
